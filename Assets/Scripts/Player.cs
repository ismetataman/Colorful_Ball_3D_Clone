...
public class Player : MonoBehaviour
{
        ...
        if(Input.touchCount > 0)
            {
                ...
                {
                    if(!EventSystem.current.IsPointerOverGameObject(Input.GetTouch(0).fingerId)) //ilk dokunuş canvas elemanı değilse oyuna başla
                    {
    ...
    public void OnCollisionEnter(Collision hit) 
    {
        if(hit.gameObject.CompareTag("Obstacles"))
        {
            ...
            StartCoroutine("TimeScaleControl"); // || startcoroutine(TimeScaleControl()); şeklinde de yazılabilir.
        }
    ...
    public IEnumerator TimeScaleControl()
    {
        ...
        Time.timeScale = 0.4f; //çaprtığında zamanı yavaşlatmak için
